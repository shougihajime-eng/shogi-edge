...
- 共有 Supabase に `shogi_edge` スキーマと 11 テーブル + RLS + GRANT + 初期重み を作成
...
- シードデータ: 主要棋士 6 名 (藤井聡太・永瀬拓矢・渡辺明・豊島将之・羽生善治・中村太地) + デモ対局 1 件
- デモ対局の予想生成成功 (★1・レーティングのみで他は「判断保留」=推測禁止ルール通り)

### 🟡 進行中
- (なし)

### 🔜 次の一歩
1. (任意) Vercel に `ANTHROPIC_API_KEY` を登録 → 一言サマリーが「はじめ先生口調」に進化
   - 登録方法: Vercel ダッシュボード → shogi-edge → Settings → Environment Variables → 追加
2. 管理画面で本物の段位/成績/直接対戦を入力していく (信頼度★が自動で上がる)
3. 連盟サイト復旧 + 規約OK確認後に B層スクレイパーを 1 サイトずつ ON
