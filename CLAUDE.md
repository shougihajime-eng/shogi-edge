...
### ✅ 直近で済んだこと (2026-05-22 Wikipedia 自動同期 + 自己学習・振り返り)
- **【最新】Wikipedia 自動同期 (規約クリア・毎週稼働)**:
  - 各サイト規約調査 → 連盟系は 503 で判定不能 / 商用サイトは原則 NG / **Wikipedia (CC-BY-SA) と Wikidata (CC0) は堂々と使える**と確定
  - 「将棋棋士一覧」記事から現役 169 名の段位・出身・師匠・生年月日を抽出するパーサーを実装 (cheerio)
  - `src/lib/wiki/{client,parser,sync}.ts` + `/api/cron/wiki-sync` で毎週月曜 03:00 JST に自動同期
  - 既存棋士は段位差分があれば update、新規はデフォルト rating 1500 で投入 (**手調整した rating は触らない**)
  - 管理画面 `/admin/wiki-sync` で「いますぐ実行」+ 実行履歴を表示
  - マイグレーション `0003_wiki_sync.sql` で `players` に wikidata_qid / wikipedia_url / wiki_synced_at / player_number / is_retired を追加、`wiki_sync_logs` テーブルを新設
...
### 🟡 進行中
- Wikipedia 由来の直接対戦 171局を CSV 化済み・本番DB への貼り付け待ち
  - 出力先: `data/head_to_head_wikipedia.csv`
  - 内訳: 藤井聡太タイトル戦番勝負 112局 + 第82期A級順位戦 21局 + 第83期A級順位戦 20局 + 第82期B級1組順位戦 9局 + 第83期B級1組順位戦 9局
...
4. 🟡 `data/head_to_head_wikipedia.csv` (153局) を `/admin/import` に貼り付け → 信頼度★が一気に上がる
5. 他棋戦(B級順位戦・新人王戦など)の追加データ取得 (希望時)
6. 連盟サイト復旧 + 規約OK確認後に各スクレイパーを `enabled=true` にして順次 ON

### 🧰 メンテ用エンドポイント
- `POST /api/seed-demo` — ヘッダ `x-admin-password: <合言葉>` で叩くとシードを再投入できる(upsert)
- `POST /api/demo-finalize` — `{ match_id, winner_id }` で対局結果を確定 → 自己学習発火
- `POST /api/demo-approve` — `{ monthly_id }` で月次重み調整を承認 → weight_history 更新

### 📘 スクレイパー有効化 (B層) — コード変更不要で ON/OFF
- **手順書**: [docs/scraper-enablement-plan.md](docs/scraper-enablement-plan.md) — P1〜P4 優先度・規約チェック・連絡先まで網羅
- **画面**: `/admin/scraper-status` で優先度バッジ・規約リスク・環境変数名を常時確認
- **仕組み**: 全 10 ルートが `src/lib/scrapers/registry.ts` を単一ソースに参照。`SCRAPER_xxx_ENABLED=true` を Vercel 環境変数に追加 → Redeploy で有効化
- **ABEMA は永久 OFF** (registry に `priority: 99` で固定。環境変数を設定しても OFF のまま)
