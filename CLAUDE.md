...
### 🔜 次の一歩
1. (任意) Vercel に `ANTHROPIC_API_KEY` を登録 → Claude による一言サマリーが本物の口調に
2. 管理画面で棋士データ・直近成績・直接対戦の手動投入 (10〜20名程度から)
3. 対局を 1 件登録 → 「予想を生成」ボタンで動作確認
4. 連盟サイト復旧 + 規約OK確認後に B層を 1 サイトずつ ON
